...
final class DFCodeEditor: NSView {
  ...
  private func applySyntaxHighlighting(to text: String, language: String) -> NSAttributedString {
    ...
    // Block comments: /* ... */
    applyPattern(
      #"/\*[\s\S]*?\*/"#, to: result, in: fullRange, color: Theme.text3, dotMatchesNewlines: true)
    ...
    applyPattern(
      #"(?<=\bas\s)\b[A-Z][A-Za-z0-9_]*\b"#, to: result, in: fullRange, color: Theme.cyan)
    applyPattern(
      #"(?<=class\s)\b[A-Z][A-Za-z0-9_]*\b"#, to: result, in: fullRange, color: Theme.cyan)
    applyPattern(
      #"(?<=struct\s)\b[A-Z][A-Za-z0-9_]*\b"#, to: result, in: fullRange, color: Theme.cyan)
    applyPattern(
      #"(?<=enum\s)\b[A-Z][A-Za-z0-9_]*\b"#, to: result, in: fullRange, color: Theme.cyan)
    applyPattern(
      #"(?<=protocol\s)\b[A-Z][A-Za-z0-9_]*\b"#, to: result, in: fullRange, color: Theme.cyan)
  ...
  private func keywordsFor(language: String) -> [String] {
    ...
    case "swift":
      return [
        "func", "class", "struct", "enum", "let", "var", "if", "else", "for",
    ...
    case "rust":
      return [
        "fn", "struct", "enum", "let", "mut", "if", "else", "for", "while",
  ...
  private func performSearch() {
    ...
    matchCountLabel.stringValue =
      searchMatches.isEmpty ? "No matches" : "\(currentMatchIndex + 1)/\(searchMatches.count)"
  }
